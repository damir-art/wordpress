...
## Админка > Настройки
Переходим в `Настройки > Общие`. Здесь можно изменить название сайта, описание, добавить фавиконку, поменять Е-маил администратора сайта и т.д.
...
- Другие настройки комментариев:
  ...
  - Включить цепочки (вложенные) комментарии глубиной `2` уровней.
...
- Размер миниатюры: `150 x 150`
- Средний размер: `600 x 600` (600x450)
- Крупный размер: `1024 x 1024`
...
- Далее ставят какой-нить плагин который переводит кириллические символы в адресе страницы в латиницу, например `Cyr-To-Lat`:
  ...
  - Настройки > Конвертер > Конвертировать существующие ярлыки
...
Новые инструкции добавляем после строки `define( 'WP_DEBUG', false );`, между комментариями на 91 строке. В начале можно выставить две инструкции: отключить ревизии и увеличить параметр автосохранения. Либо не добавлять ничего.
    ...
    // Включить показ ошибок (false меняем на true на 88 строке)
...
Начальный robots.txt, дальше по ситуации:
    ...
    # Адрес на встроенную карту XML
    Sitemap: http://example.com/wp-sitemap.xml

    # Версия кода: 2.0
    # Не забудьте поменять `example.com` на ваш сайт.

Также с версии WordPress 5.5 по-умолчанию установливается XML-карта сайта, поэтому ставить ради неё дополнительные плагины не имеет смысла.
