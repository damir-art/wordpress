...
Для создания вложенного меню с помощью Bootstrap, есть два способа: 
- взять CSS-классы Bootstrap и применить их к к дефолтным классам меню WordPress которые создаются на автомате
- создать свой класс построения меню, на основе Walker_Nav_Menu{}, более надёжный способ, для сложных меню, где переопределяем методы:
